defmodule ExPostFacto.EnhancedValidationTest do
  ...
  describe "enhanced validation system" do
    test "backtest with enhanced_validation: true works for valid data" do
      ...
      assert {:ok, output} = ExPostFacto.backtest(data, {Noop, :noop, []}, enhanced_validation: true)
    ...
    test "enhanced validation catches invalid data with detailed errors" do
      ...
      invalid_data = [
        %{open: 10.0, high: 10.5}, # Missing low, close
        %{low: 9.5, close: 10.2}   # Missing open, high
      ]
    ...
    test "enhanced validation catches invalid strategy with detailed errors" do
      ...
      assert {:error, %Validation.StrategyError{} = error} =
        ExPostFacto.backtest(data, {NonExistentModule, :missing, []}, enhanced_validation: true)
    ...
    test "debug mode provides detailed logging" do
      ...
      ExUnit.CaptureIO.capture_io(fn ->
        {:ok, _} = ExPostFacto.backtest(data, {Noop, :noop, []},
    ...
    test "warning system provides runtime feedback" do
      ...
      assert {:ok, _output} = ExPostFacto.backtest(data, {Noop, :noop, []},
                                                  enhanced_validation: true,
                                                  warnings: true)
    end
    ...
    test "enhanced validation is backward compatible" do
      ...
      assert {:ok, output2} = ExPostFacto.backtest(data, {Noop, :noop, []}, enhanced_validation: false)

      # Should work with enhanced_validation: true
      assert {:ok, output3} = ExPostFacto.backtest(data, {Noop, :noop, []}, enhanced_validation: true)
  ...
  describe "Validation module" do
    test "validate_data/2 handles various data formats" do
      ...
      assert result in [:ok, {:warning, "No volume data detected - some strategies may require volume information"}]
    ...
    test "validate_strategy/2 handles MFA tuples" do
      ...
      assert result in [:ok, {:warning, "Using no-operation strategy - this is typically for testing only"}]
