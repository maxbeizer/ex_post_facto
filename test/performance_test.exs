defmodule ExPostFacto.PerformanceTest do
  ...
  describe "streaming indicators" do
    test "SMA streaming processor works correctly" do
      ...
      results = [
        ...
        # Should return 20.0 (average of 10,20,30)
        ExPostFacto.Indicators.Streaming.SMA.update(sma_pid, 30),
        # Should return 30.0 (average of 20,30,40)
        ExPostFacto.Indicators.Streaming.SMA.update(sma_pid, 40)
      ]
    ...
    test "EMA streaming processor works correctly" do
      ...
      results = [
        # Should return 10.0 (first value)
        ExPostFacto.Indicators.Streaming.EMA.update(ema_pid, 10),
        # Should be between 10 and 20
        ExPostFacto.Indicators.Streaming.EMA.update(ema_pid, 20),
        # Should be between previous and 30
        ExPostFacto.Indicators.Streaming.EMA.update(ema_pid, 30)
      ]
    ...
    test "batch processing multiple indicators" do
      ...
      {:ok, results} =
        ExPostFacto.Indicators.Streaming.process_batch(
          prices,
          [
            {:sma, [period: 3]},
            {:ema, [period: 3]},
  ...
  describe "performance comparison" do
    ...
    test "demonstrates performance improvement with larger datasets" do
      ...
      {:ok, _result} =
        Optimizer.grid_search(
          # Use subset for optimization test
          Enum.take(large_data, 1000),
          ExPostFacto.ExampleStrategies.SmaStrategy,
      ...
      assert backtest_time < 10_000
      # Less than 30 seconds
      assert optimization_time < 30_000
    end
  end
