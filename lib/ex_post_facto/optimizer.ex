defmodule ExPostFacto.Optimizer do
    ...
    case generate_parameter_combinations(param_ranges, max_combinations) do
      {:ok, combinations} ->
        ...
        case find_best_result(results, metric) do
          ...
          {:error, _reason} ->
            # Handle case where all backtests failed gracefully
            {:ok,
             %{
               best_params: nil,
               best_score: nil,
  ...
  def walk_forward(data, strategy_module, param_ranges, opts \\ []) do
    ...
    if length(data) < window_size do
      {:error,
       "Insufficient data for walk-forward analysis. Need at least #{window_size} data points."}
    else
      {:ok, windows} =
  ...
  defp generate_parameter_combinations(param_ranges, max_combinations) do
    try do
      ...
      if length(combinations) > max_combinations do
        {:error,
         "Too many parameter combinations (#{length(combinations)}), limit is #{max_combinations}"}
      else
        {:ok, combinations}
      end
    rescue
  ...
  defp extract_metric_score(result, metric) do
    case metric do
      :sharpe_ratio ->
        Map.get(result, :sharpe_ratio, 0.0)

      :total_return_pct ->
        Map.get(result, :total_return_pct, 0.0)

      :cagr_pct ->
        Map.get(result, :cagr_pct, 0.0)

      :profit_factor ->
        Map.get(result, :profit_factor, 0.0)

      :sqn ->
        Map.get(result, :sqn, 0.0)

      :win_rate ->
        Map.get(result, :win_rate, 0.0)
  ...
  defp generate_heatmap_data(result, x_param, y_param) do
    ...
    data_points =
      Enum.map(result.all_results, fn %{params: params, score: score} ->
  ...
  defp run_walk_forward_windows(
    ...
    windows =
      for start_idx <- 0..(data_length - window_size)//step_size do
        ...
        case optimization_result do
          {:ok, opt_result} ->
            ...
            validation_opts =
              Keyword.drop(optimization_opts, [:maximize, :max_combinations, :samples])

            case ExPostFacto.backtest(
                   validation_data,
                   {strategy_module, best_params},
                   validation_opts
                 ) do
  ...
  defp analyze_parameter_stability(windows) do
    ...
    if Enum.empty?(valid_windows) do
      ...
      stability_metrics =
        Enum.map(all_params, fn param_name ->
          values =
            Enum.map(valid_windows, fn %{best_params: params} ->
  ...
  defp calculate_overall_stability(stability_metrics) when map_size(stability_metrics) == 0,
    do: 0.0
